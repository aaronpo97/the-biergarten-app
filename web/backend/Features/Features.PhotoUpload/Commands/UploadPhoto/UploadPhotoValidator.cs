...
public class UploadPhotoValidator : AbstractValidator<UploadPhotoCommand>
{
    ...
    public UploadPhotoValidator()
    {
        ...
        RuleFor(x => x.File.Length)
            .InclusiveBetween(MinSizeBytes, MaxSizeBytes)
            .WithMessage("The file must be between 1 KiB and 24 MiB.");
    ...
    private static async Task<bool> HaveValidImageSignatureAsync(
        IFormFile file,
    ...
    private static bool IsJpeg(byte[] buffer, int bytesRead) =>
        bytesRead >= JpegSignature.Length
