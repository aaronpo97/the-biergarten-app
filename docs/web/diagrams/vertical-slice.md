...
- **API.Core**: Program.cs wiring only
  ...
  - Only `Features.Users` and `Features.Breweries` get `AddApplicationPart`
    (controllers)
  - `Features.PhotoUpload` and `Features.Emails` scanned for MediatR handlers
    only
  - `Features.Locations` and `Features.Beers` not referenced by `API.Core` at
    all

- **Feature slices**: each owns its own controller (where present),
  ...
  - `Features.Locations` has no controller; one MediatR handler
    (`GetOrCreateCityHandler`) registered by `Database.Seed` only — `API.Core`
    never scans it
...
- **Domain.Entities**:
  - `UserAccount`, `UserCredential`, `UserVerification`, `UserProfile`,
    `UserAvatar`
  - `BreweryPost`, `BreweryPostLocation`
  - `City`, `StateProvince`, `Country`
  - `BeerPost`, `BeerStyle`, `Photo` (schema tables exist for
    `BeerPost`/`BeerStyle`; no repository yet since `Features.Beers` has no
    code)
...
- **Database.Migrations**: separate console app
  ...
  - distinct from `Database.Seed`, which loads data through the feature slices'
    own DI extensions
